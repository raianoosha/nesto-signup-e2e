...
| Verify a password violating "tooLong" is rejected | Generated test; checks *some* error shows (exact wording unconfirmed) |
...
| Verify signing up twice with the same email does not silently succeed a second time | **Mutating**, `test.slow()` (30s timeouts) — signs up once (expects 201), then again with the same email (expects anything *but* 201) |
